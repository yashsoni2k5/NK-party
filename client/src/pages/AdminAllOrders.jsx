            ...
            <button
              ...
              className={`px-4 py-2 rounded-2xl text-xs font-extrabold transition-all duration-200 border flex items-center gap-2 ${
                activeFilter === tab.id
                  ...
                  : 'bg-black/30 hover:bg-black/60 text-[#FAF7F0] border-[#E3BA63]/20 hover:border-[#E3BA63]/50'
              }`}
            >
              <span>{tab.label}</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${
                activeFilter === tab.id ? 'bg-[#011E15] text-[#E3BA63]' : 'bg-[#011E15] text-[#E3BA63]'
              }`}>
                {statusCounts[tab.id]}
              </span>
            </button>
            ...
            {filteredOrders?.map(order => (
              ...
              >
                ...
                <div className="p-6 border-b border-[#E3BA63]/20 bg-black/20 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                  <div className="space-y-1">
                    <p className="text-xs font-semibold text-[#FAF7F0] uppercase tracking-widest">Order ID</p>
                    <p className="text-[#FAF7F0] font-mono font-bold text-sm sm:text-base">{order._id}</p>
                    <p className="text-xs text-[#FAF7F0] flex items-center gap-1 mt-1">
                      <FiUser className="text-[#E3BA63]" /> User ID: <span className="font-mono text-[#FAF7F0]">{order.user?._id || order.user}</span>
                    </p>
                  </div>

                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
                    <div className="text-left sm:text-right">
                      <p className="text-xs text-[#FAF7F0] uppercase tracking-wider">Total Value</p>
                      <p className="text-[#E3BA63] font-extrabold text-xl">₹{order.total}</p>
                    </div>

                    <div className="space-y-1">
                      <label className="block text-[10px] font-bold uppercase tracking-wider text-[#FAF7F0]">Order Status</label>
                      <select 
                        value={order.status} 
                ...
                <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {order.deliveryAddress ? (
                    <div className="bg-[#011E15] border border-[#E3BA63]/20 p-4 rounded-2xl space-y-2 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-[#FAF7F0] text-xs">{order.deliveryAddress.house_no}, {order.deliveryAddress.area}</p>
                        <p className="text-[#FAF7F0] text-xs">{order.deliveryAddress.city} - {order.deliveryAddress.pincode}</p>
                      </div>
                      
                      {/* Refund UI */}
                      {order.refundStatus === "NONE" && (order.status === "CANCELLED" || order.status === "DELIVERED") && (
                        <div className="mt-4 pt-4 border-t border-[#E3BA63]/20">
                          <p className="text-[10px] font-bold text-[#FAF7F0] uppercase tracking-wider mb-2">Process Refund</p>
                      ...
                      {order.refundStatus !== "NONE" && (
                        <div className="mt-4 pt-4 border-t border-[#E3BA63]/20">
                           <p className="text-[10px] font-bold text-[#FAF7F0] uppercase tracking-wider mb-1">Refund Status</p>
                           <p className="text-sm font-bold text-teal-400">{order.refundStatus.replace('_', ' ')}</p>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="bg-[#011E15] border border-[#E3BA63]/20 p-4 rounded-2xl text-xs text-[#FAF7F0] flex items-center justify-center">
