...
export default function Cart() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#003725] flex justify-center items-center text-[#E3BA63]">
        ...
        <span className="font-bold text-lg text-[#FAF7F0]">Loading your luxury cart...</span>
      </div>
    );
  }
  ...
  if (!cartItems?.length) {
    return (
      <div className="min-h-screen bg-[#003725] py-16 px-4 flex items-center justify-center">
        <div className="max-w-lg w-full bg-[#011E15] border border-[#E3BA63]/30 rounded-3xl p-8 sm:p-12 text-center shadow-2xl">
          ...
          <button 
            onClick={() => navigate('/')} 
            className="w-full py-4 bg-[#E3BA63] hover:bg-[#cda24d] text-[#011E15] font-extrabold text-base rounded-xl transition-all duration-200 shadow-lg active:scale-95 flex items-center justify-center gap-2"
          >
            <span>🛍️</span>
  ...
  return (
    <div className="min-h-screen bg-[#003725] text-[#FAF7F0] py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Header */}
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-[#E3BA63]/20">
          <h1 className="text-2xl sm:text-4xl font-extrabold text-[#FAF7F0] flex items-center gap-3">
          ...
          <Link to="/" className="text-xs sm:text-sm font-semibold text-[#E3BA63] hover:underline flex items-center gap-1">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-4">
            {cartItems.map((item) => {
              ...
              return (
                <div 
                  key={item._id} 
                  className="bg-[#011E15] border border-[#E3BA63]/30 p-4 sm:p-5 rounded-2xl shadow-xl flex flex-col sm:flex-row sm:items-center gap-4 transition-all hover:border-[#E3BA63]/60"
                >
                  {/* Image */}
                  <div className="w-24 h-24 sm:w-28 sm:h-28 bg-black/40 rounded-xl overflow-hidden border border-[#E3BA63]/20 flex-shrink-0 mx-auto sm:mx-0 flex items-center justify-center">
                    {p.image ? (
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    <h3 className="font-bold text-base sm:text-lg text-[#FAF7F0] mb-1 line-clamp-1">
                  ...
                  <div className="flex flex-col sm:items-end gap-3 pt-3 sm:pt-0 border-t sm:border-t-0 border-[#E3BA63]/10">
                    
                    {/* Quantity Adjustment Selector */}
                    <div className="flex items-center justify-center bg-black/40 border border-[#E3BA63]/40 rounded-xl p-1 shadow-inner">
                      <button 
                        onClick={() => handleUpdateQuantity(item.productId, 'dec')}
                        disabled={isUpdating}
                        title={item.quantity === 1 ? "Remove product" : "Decrease quantity"}
                        className="w-8 h-8 rounded-lg bg-[#003725] text-[#E3BA63] hover:bg-[#E3BA63] hover:text-[#011E15] font-extrabold text-lg flex items-center justify-center transition-colors disabled:opacity-40"
                      >
                        -
                      </button>
                      
                      <span className="w-10 text-center font-extrabold text-[#FAF7F0] text-sm sm:text-base">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-4 w-full">
                      ...
                      <button 
                        ...
                        disabled={isUpdating}
                        className="text-xs font-semibold text-red-400 hover:text-red-300 hover:underline flex items-center gap-1 disabled:opacity-50"
                      >
                        <span>🗑️</span> Remove
                      </button>
                    </div>
          ...
          {/* Cart Summary Card */}
          <div className="w-full lg:w-96 bg-[#011E15] border border-[#E3BA63]/30 p-6 rounded-3xl shadow-2xl h-fit sticky top-24">
            <h2 className="text-xl font-extrabold text-[#E3BA63] border-b border-[#E3BA63]/20 pb-4 mb-4 flex items-center gap-2">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="border-t border-[#E3BA63]/20 pt-3 flex justify-between items-end">
                <span className="text-base font-bold text-[#FAF7F0]">Estimated Total</span>
                <span className="text-2xl font-extrabold text-[#E3BA63]">₹{cartTotal}</span>
              </div>
            </div>

            {/* Minimum Order Warning */}
            {cartTotal < 500 && (
              <div className="bg-red-500/15 border border-red-500/30 p-3.5 rounded-xl mb-6 text-xs text-red-300 leading-relaxed flex items-start gap-2">
