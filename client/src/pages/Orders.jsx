  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex justify-center items-center text-[#AC666D]">
        ...
        <span className="font-bold text-lg text-gray-800">Loading your orders...</span>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-gray-800 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col gap-6">
          {orders?.map(order => (
            <div key={order._id} className="bg-white border border-[#AC666D]/30 rounded-2xl shadow-xl overflow-hidden hover:border-[#AC666D]/60 transition-colors duration-300">
              
              {/* Order Header */}
              <div className="p-5 sm:p-6 border-b border-[#AC666D]/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50">
                <div>
                  <p className="text-xs font-semibold text-gray-800 uppercase tracking-widest mb-1">Order ID</p>
                  <p className="text-gray-800 font-mono text-sm sm:text-base">{order._id}</p>
                </div>
                <div className="flex flex-col sm:items-end gap-2">
                  ...
                  <div className="text-right">
                    <p className="text-xs text-gray-800 uppercase tracking-wider">Total</p>
                    <p className="text-[#AC666D] font-extrabold text-lg">₹{order.total}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 sm:p-6 flex flex-col gap-5">
                {order.products?.map((item, i) => (
                  <div key={i} className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    <div className="w-20 h-20 bg-gray-100 rounded-xl overflow-hidden border border-[#AC666D]/20 flex-shrink-0 flex items-center justify-center">
                      {item.product?.image ? (
                    ...
                    <div className="flex-grow">
                      <strong className="block text-gray-800 font-bold text-base mb-1">{item.product?.title || 'Product Unavailable'}</strong>
                      <div className="flex items-center gap-3 text-sm text-gray-800">
                        <span>Qty: <span className="font-bold text-gray-800">{item.quantity}</span></span>
                        <span>•</span>
                        <span>Price: <span className="font-bold text-[#AC666D]">₹{item.priceAtPurchase}</span></span>
                      </div>
                    </div>
              ...
              {/* Order Footer */}
              <div className="p-5 sm:p-6 border-t border-[#AC666D]/20 bg-black/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="text-sm text-gray-800">
                  <span className="font-semibold text-gray-800">Delivering to:</span> {order.deliveryAddress?.house_no}, {order.deliveryAddress?.city}
                </div>
                
                {(order.status === 'PENDING' || order.status === 'PROCESSING') && (
                  <button 
                    onClick={() => handleCancelOrder(order._id)}
                    className="px-5 py-2.5 bg-red-500/10 hover:bg-red-500 text-red-600 hover:text-white border border-red-500/30 rounded-xl font-bold transition-all text-sm w-full sm:w-auto"
                  >
                    Cancel Order
                  </button>
                )}
